...
if __name__ == "__main__":
    ...
    data_struct = mat73.loadmat(os.path.join('data',sample,sample+'_'+color+'.mat'))
    ...
    for epoch in t:
        ...
        if fit_3D:
            dzs = ( (torch.randperm(num_z-1)[:num_z//2] + torch.rand(num_z//2)) * ((z_max-z_min)//(num_z-1))  ).to(device) + z_min
            if epoch % 2 == 0:
